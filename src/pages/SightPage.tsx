...
const SightPage: React.FC = () => {
    const { slug } = useParams<{ slug: string }>();
    const article = sights.find(a => a.id === slug);

    // Unknown slug → back to the index rather than a dead page.
    if (!article) return <Navigate to="/sights" replace />;
    ...
    return (
        <div className="min-h-screen bg-transparent pb-24">
            ...
            <article className="max-w-4xl mx-auto px-6 pt-6">
                ...
                >
                    <ArrowLeft className="w-4 h-4" /> Все достопримечательности
                </Link>
            </article>
        </div>
    );
